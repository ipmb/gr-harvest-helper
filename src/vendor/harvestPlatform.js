...
(function () {
  ...
  config = function () {
    if (window._harvestPlatformConfig) {
      ...
      const platformConfigElement = document.querySelector('script[data-platform-config]');
      const _harvestPlatformConfig = {
        applicationName: 'UnoficialTimeTrackerExtension',
        skipStyling: true,
      };
      if (platformConfigElement && platformConfigElement.dataset.platformConfig) {
        try {
          return JSON.parse(platformConfigElement.dataset.platformConfig);
        } catch (error) {
          console.error('Failed to parse platformConfig:', error);
          return _harvestPlatformConfig; // Return a default fallback object
        }
      } else {
        console.warn('Platform config script not found or missing dataset. Using default config.');
        return _harvestPlatformConfig; // Return a default fallback object
      }
    }
  };
  ...
  setTimer = function (data) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      if (
        data == null ||
        (group != null ? group.id : void 0) !== ((ref1 = data.group) != null ? ref1.id : void 0) ||
        (item != null ? item.id : void 0) !== ((ref2 = data.item) != null ? ref2.id : void 0)
      ) {
